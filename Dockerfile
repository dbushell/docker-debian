FROM debian:12.0-slim as debian-base
...
RUN useradd -u ${PUID} -U -d ${HOME} -s /usr/zsh ${USER} \
...
FROM debian-base as debian-node
...
# Install Deno
FROM debian-node as debian-deno
...
# Install Bun
FROM debian-deno as debian-bun
